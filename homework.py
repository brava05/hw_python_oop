...
class Swimming(Training):
    ...
    def get_mean_speed(self) -> float:
        ...
        # в read_package() тоже добавил проверку, но сюда возможно будут
        # попадать из других мест данные потом )
        if self.duration == 0:
            return 0
        else:
...
def read_package(workout_type: str, data: list) -> Training:
    ...
    if data[1] == 0:
        raise('Продолжительность тернировки не может быть равно нулю')

    # создаем объект нужного типа (когда-то это будет очевидно из кода,
    # пока для меня это магия: создать объект по значению какой-то переменной)
